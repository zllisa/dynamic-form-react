...
const EditCheckBox: React.FC<EditCheckBoxProps> = (props) => {
    ...
    const [title,setTitle] = useState<string>(props.title||'多选题')
    const [required,setRequired] = useState<boolean>(props.required!==undefined?props.required:false)
    const [isLimited,setIsLimited] = useState<boolean>(props.isLimited!==undefined?props.isLimited:false)
    const [optionsLimited,setOptionsLimited] = useState<number>(props.optionsLimited||3)
    const [hasOtherItem, setHasOtherItem] = useState<boolean>(false)


    const handleReuqiredChange = (checked: boolean) => {
        setRequired(checked)
        updateData('required',checked)
    }
    const handleOptionsLimitedChange = (checked: boolean) => {
    ...
    return (
        <div
            className={`edit_checkbox ${props.className ? props.className : ''}`}
            style={props.style}
        >
            <div className="title">多选题</div>
            <div className="body">
                <div className="type">
                    <span>题目</span>
                ...
                <div className="options">
                    ...
                    <div className="required">
                        <span>必填</span>
                        <Switch checked={required} onChange={handleReuqiredChange} />
                    </div>
                    <div className="options_limited">
                        <span>多选数目限制</span>
                        <Switch checked={isLimited} onChange={handleOptionsLimitedChange} />
                    </div>
                </div>
            </div>
        </div>
